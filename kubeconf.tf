...
#____________________________CLOUD-CONTROLLER-MANAGER__________________________________________

data "template_file" "cloud-ctrl" {

    template = "${file("./kubeconf/cloud-controller-manager.tpl")}"

      vars = {

        ca_path                        = base64encode(tls_self_signed_cert.kube_ca.cert_pem)
        cloud-controller-manager_c_path = base64encode(tls_locally_signed_cert.cloud_controller_manager.cert_pem)
        cloud-controller-manager_k_path = base64encode(tls_private_key.cloud_controller_manager.private_key_pem)
    }

}

resource "local_file" "cloud_ctrl" {
  content  = "${data.template_file.cloud-ctrl.rendered}"
  filename = "./kubeconf/cloud-controller-manager.kubeconfig"
}
